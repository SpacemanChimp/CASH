...
function getModeBlock(row) {
  ...
  if (row.category === 'manufacturing' || row.category === 'reactions') {
    return row[MODE] || row.instant;
  }
  if (row.category === 't2') return row.instant;
  return null;
}

function getSellMarketLabel(row) {
  // Rows generated by update_rankings.py now include sell market hints.
  // We show the best sell market for the currently selected mode.
  if (!row) return "—";
  if (MODE === "patient") {
    return row.sell_patient_market_name || row.sell_patient_market || "—";
  }
  return row.sell_instant_market_name || row.sell_instant_market || "—";
}
...
function passesFilters(row) {
  ...
  if (els.validatedOnly.checked && !hasDepth(row)) return false;
...
async function loadData() {
  ...
  DATA = await r.json();
  setStatus(`Updated ${DATA.generated_at} | Region ${DATA.market?.region_id} | Mode: ${MODE}`);
}
...
function planCompute() {
  ...
  for (const p of PLAN) {
    ...
    if (row.category === 'refining') {
      ...
      // sell minerals
      for (const out of (row.materials || [])) {
        bumpMap(sellMap, out.name, Number(out.qty || 0) * runs);
      }
    } else {
      // buy inputs
      const m = getModeBlock(row);
      ...
      // sell output
      bumpMap(sellMap, row.product_name, Number(row.output_qty || 1) * runs);
